...
class Medicamento:
    ...
    def __init__(self, cn):
        ...
        self.sql_list = "SELECT IdMedicamento, Nombre, Tipo FROM medicamentos ORDER BY IdMedicamento DESC"
        self.sql_detail = "SELECT IdMedicamento, Nombre, Tipo FROM medicamentos WHERE IdMedicamento=%s"
        self.sql_insert = "INSERT INTO medicamentos(Nombre, Tipo) VALUES(%s,%s)"
        self.sql_update = "UPDATE medicamentos SET Nombre=%s, Tipo=%s WHERE IdMedicamento=%s"
        self.sql_delete = "DELETE FROM medicamentos WHERE IdMedicamento=%s"
    ...
    def save(self, form_data) -> str:
        ...
        try:
            cur = self.cn.cursor()
            if op == "new":
                cur.execute(self.sql_insert, (nombre, tipo))
                self.cn.commit()
                cur.close()
                return self._msg_success("Medicamento creado correctamente")

            if op == "act":
                cur.execute(self.sql_update, (nombre, tipo, id_))
                self.cn.commit()
                cur.close()
                return self._msg_success("Medicamento actualizado correctamente")
    ...
    def delete(self, id: int) -> str:
        try:
            ...
            curv.execute("SELECT 1 FROM recetas WHERE IdMedicamento=%s LIMIT 1", (id,))
            if curv.fetchone():
                curv.close()
                return self._msg_error("No se puede eliminar el medicamento porque está asociado a una consulta")
            curv.close()
            ...
            self.cn.commit()
            cur.close()
            return self._msg_success("Medicamento eliminado correctamente")
        except Exception as ex:
            return self._msg_error(f"Error SQL: {ex}")
